...
public class WaveView extends View
{
	...
	private LinearLayout parentLayout;//父layout
	public static float mWaterLineLevel = 15; // 水位线增长的等级
	private float OnePercent;// 计算1%水位线的高度
	Handler updateHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			// 记录平移总位移
			mMoveLen += SPEED;
			if (mLevelLine < mViewHeight){
				//这里的转换是因为水位线的高度和view高度为反比，即view高度等于水位线最低处
				
				float whiteHeight = OnePercent * mWaterLineLevel;
				float OilDrumHeight = mViewHeight-whiteHeight;
				mLevelLine = OilDrumHeight;
			}
			mLeftSide += SPEED;
			// 波形平移
	...
	protected void onAttachedToWindow() {
		...
		this.postDelayed(new Runnable(){
			@Override
			public void run() {
				mViewHeight = parentLayout.getHeight();
				OnePercent = mViewHeight/100;
			}
		}, 1);
		// 开始波动
		start();
